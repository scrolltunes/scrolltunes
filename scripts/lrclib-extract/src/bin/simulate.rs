...
static TRACK_NUMBER: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)^(?:track\s*)?\d{1,4}\s*[-.:)]\s*").unwrap()
});
...
fn main() -> Result<()> {
    ...
    while let Some(row) = rows.next()? {
        ...
        if new_title_norm != old_title_norm {
            ...
            if TRACK_NUMBER_SPACE_PREFIX.is_match(&title) ||
               (title.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false) &&
                old_title_norm.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(false) &&
                !new_title_norm.chars().next().map(|c| c.is_ascii_digit()).unwrap_or(true)) {
                track_num_fixed += 1;
            ...
            if examples.len() < 30 {
                examples.push((title.clone(), old_title_norm.clone(), new_title_norm.clone(),
                    if TRACK_NUMBER_SPACE_PREFIX.is_match(&title) { "track_num".to_string() }
                    else if BRACKET_SUFFIX.is_match(&title) { "bracket".to_string() }
                    else if FILE_EXTENSION.is_match(&title) { "extension".to_string() }
                    else if YEAR_SUFFIX.is_match(&title) { "year".to_string() }
    ...
    println!("Title normalization changed:  {} ({:.2}%)", title_changed, 100.0 * title_changed as f64 / total as f64);
